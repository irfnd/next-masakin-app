...
export default function PopularRecipesList({ isHome = false }) {
	...
	if (isHome)
		return (
			<>
				{data.length > 0 &&
					data.slice(0, 5).map((recipe) => <RecipesList recipe={recipe} type="popular" key={recipe.id} />)}
			</>
		);
	return (
		<>
			{data.length > 0 && data.map((recipe) => <RecipesList recipe={recipe} type="popular" key={recipe.id} withBtn />)}
		</>
	);
}
