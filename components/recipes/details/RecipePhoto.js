...
export default function RecipePhoto({ id }) {
	...
	// Attributes
	const photoUrl = `linear-gradient(rgba(0, 0, 0, 0), rgba(0, 0, 0, 0.4))`;
	const photoStyle = { height: "50%", backgroundImage: photoUrl, backgroundSize: "cover" };

	return (
		<div className="d-flex flex-column justify-content-center align-items-center gap-3 w-100 p-4 pb-5" style={photoStyle}>
			<div className="position-relative d-flex justify-content-between align-items-end pb-3 mb-5 w-100 h-100">
				<BackBtn style={{ top: 0 }} />
